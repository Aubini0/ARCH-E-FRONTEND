...
import { AnimatePresence, MotionValue, animate, motion, useAnimation, useMotionValue, useSpring, useTransform } from "framer-motion";
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-end h-14 p-2 gap-3  bg-opacity-70 rounded-xl",
          "bg-secondary/70 p-2 no-underline shadow-sm transition-colors",
...
function DockCardInner({ src, id, children }: DockCardInnerProps) {
  ...
  return (
    <span className="relative flex justify-center items-center z-0 overflow-hidden w-full h-full rounded-md">
      ...
      <motion.img className="absolute z-10 opacity-40 filter blur-md transform translate-y-2.5 scale-125 " src={src} alt="" />
...
function DockDivider() {
  return (
    <motion.div className="h-full flex items-center p-1.5 cursor-ns-resize" drag="y" dragConstraints={{ top: -100, bottom: 50 }}>
