...
const DrawerContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content> & {
    swapper?: boolean;
    swapperClassName?: React.HTMLAttributes<HTMLDivElement>["className"];
  }
>(({ className, children, swapper = true, swapperClassName, ...props }, ref) => (
  <DrawerPortal>
    <DrawerOverlay />
    ...
    >
      {swapper && (
        <div className="absolute top-0 left-0 w-full">
          <div className={cn("mx-auto mt-4 h-[8px] w-[100px] rounded-full dark:bg-secondary dark:text-white bg-gray-400 text-black", swapperClassName)} />
        </div>
      )}
      {children}
    </DrawerPrimitive.Content>
...
const DrawerHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => <div className={cn("grid gap-1.5 px-4 py-0 text-center sm:text-left", className)} {...props} />;
