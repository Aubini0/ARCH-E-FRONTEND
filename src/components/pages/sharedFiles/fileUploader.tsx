...
import { AudioWaveform, File, FileImage, FolderArchive, ImageIcon, UploadCloud, Video, X } from "lucide-react";
...
export default function ImageUpload() {
  ...
  const getFileIconAndColor = (file: File) => {
    if (file.type.includes(FileTypes.Image)) {
      return {
        icon: <ImageIcon color="gray" size={40} />,
  ...
  return (
    <div>
      ...
      {filesToUpload?.length ? <div className="flex justify-end mt-5 w-full">
          <button className="dark:bg-[#3D3D3D] p-[6px_24px_6px_24px] bg-[#efefef] rounded-[48px]">Save</button>
        </div> : <></>}
    </div>
  );
}
